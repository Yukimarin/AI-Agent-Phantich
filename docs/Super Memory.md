...
- [2026-07-09] **Tích hợp Obsidian Vault**: Thống nhất cấu trúc tiêu đề lớp (`### Lớp: HN-KS24-CNTT1`) và tự động chèn Wiki-links liên kết chéo từ báo cáo KPI GV/TG sang Care List của từng lớp học trong Obsidian. Tạo tài liệu hướng dẫn [[Hướng dẫn sử dụng Obsidian|HOW_TO_USE_OBSIDIAN.md]]. Cài đặt và kích hoạt sẵn plugin Dataview và Admonition thông qua script tải GitHub và cấu hình file `.obsidian/community-plugins.json`.
...
- [2026-07-16] **Chuẩn hóa Wiki-links & Đồng bộ GitHub**:
  - Chuyển đổi toàn bộ liên kết tĩnh sang Wiki-links Obsidian tương đối (dạng `[[Super Memory]]`, `[[Báo cáo KPI Giảng viên Trợ giảng]]`), khắc phục đứt gãy để kích hoạt thành công Graph view.
...
- [2026-07-16] **Thiết lập Bản đồ Tri thức (MOC)**:
  ...
  - Phát triển script `scratch/link_all_nodes.py` tự động chèn backlinks `[[Bản đồ Tri thức MOC|Bản đồ Tri thức dự án]]` cho toàn bộ các ghi chú con rời rạc thành công và đồng bộ lên GitHub.
...
- [2026-07-17] **Đánh giá Năng lực theo Tiêu chuẩn mới V2**:
  * Đã so sánh chi tiết và làm nổi bật các thay đổi của chuẩn V2 so với bản cũ (bổ sung nhóm Rank 5-6 GV Thương hiệu, loại bỏ 20% trọng số sư phạm của Trợ giảng để dồn vào hỗ trợ vận hành và đôn đốc, phân tách các tiêu chí Elearning vs BTVN, Pass vs Khá giỏi).
  * **Phát hiện & Khắc phục lệch dòng Trợ giảng (TA) trong Excel**: Do hàng Trợ giảng trong Excel chỉ ghi tên ở cột 3 và để trống các cột chỉ số phía sau, thuật toán cũ bị lỗi đọc ra `None` và hiểu sai là vi phạm 0%. Đã khắc phục bằng cách đọc chỉ số lớp học cho Trợ giảng đồng bộ trực tiếp từ dòng Giảng viên chính (`row=r` thay vì `row=r+1`).
  * Thực hiện đánh giá lại toàn bộ 33 nhân sự khối CNTT & QTKD và xuất chi tiết điểm thành phần cho 6 nhân sự yêu cầu (bao gồm phân tích đối chiếu thăng hạng của thầy Lâm Tùng Dương). Lưu báo cáo tại artifact kpi_v2_evaluation_report.md.
  * Cập nhật ngưỡng phân loại Điểm quá trình thực tế theo yêu cầu mới của User: Vượt mức (≥10.0), Đạt (7.5 - 10.0), Cần cố gắng (5.0 - 7.5), Không đạt kỳ vọng (<5.0). Điều này dịch chuyển xếp loại của nhiều nhân sự (ví dụ Lâm Tùng Dương và Phạm Viết Hùng chuyển sang "Cần cố gắng" do điểm ở dải [7.0, 7.5)).
- [2026-07-17] **Cập nhật dữ liệu mới nhất & Làm sạch Nhân sự mẫu**:
  * Đã import tệp dump SQL mới nhất `qldt_el-07-17-26.sql` vào MySQL `qldt_el` cổng 3307 và chép đè tệp Excel mới nhất `PTIT_Chiso.xlsx` từ Backup vào dự án.
  * Xóa hoàn toàn các nhân sự mẫu "Nguyễn Văn A" và "Trần Thị B" khỏi các script `generate_kpi_report.py`, `generate_agent4_report.py`, và `analyze_daily_logs_mcp.py` để làm sạch báo cáo và Master Portal.
  * Tích hợp bước chạy `generate_kpi_ranking.py` trực tiếp vào pipeline `run_pipeline.py` trước khi sinh dashboard để pipeline hoạt động độc lập và cập nhật tự động toàn diện. Chạy lại pipeline thành công 100%, đồng bộ tệp `unified_dashboard.html` sạch sẽ ở gốc thư mục.
- [2026-07-17] **Tái cấu trúc và Đồng nhất Liên kết Obsidian Vault**:
  * Đã đổi tên toàn bộ các ghi chú trong thư mục `docs/` (bao gồm `agents/` và `plans/`) từ dạng snake_case sang dạng Title Case tiếng Việt & tiếng Anh chuyên nghiệp (ví dụ: `Bản đồ Tri thức MOC.md`, `Super Memory.md`, `Agent 1 - Violation Analyst.md`...).
  * Tạo các tệp Proxy Notes trung gian (`docs/Báo cáo Nguy cơ Học viên.md`, `docs/Báo cáo KPI Giảng viên Trợ giảng.md`...) sử dụng cú pháp nhúng `![[...]]` của Obsidian để liên kết các tệp báo cáo động của data/ mà không làm hỏng đường dẫn ghi đè cứng của pipeline Python.
  * Quét và cập nhật toàn bộ wikilinks (`[[...]]`) trên toàn bộ workspace để trỏ chính xác về cấu trúc tên file mới, đảm bảo Graph View kết nối chặt chẽ và không còn node mồ côi (orphaned nodes).
  * Xóa bỏ các tệp tin tạm rác (`Untitled.base`, `Untitled.canvas`) ở thư mục gốc.
  * Cấu hình loại trừ index các thư mục hệ thống rác (`.venv`, `.git`, `.gemini`, `scratch`, `.agents`) trong `.obsidian/app.json` để làm sạch Graph View.
  * Làm sạch tệp tin `.obsidian/workspace.json` để loại bỏ toàn bộ ghost tabs và history trỏ về `knowledge_map.md` và `Untitled` cũ.
  * Lập trình quét tự động và liên kết thành công 8 tệp ghi chú Design còn thiếu trong `docs/plans/` vào cuối tệp `Bản đồ Tri thức MOC.md` để đảm bảo 100% tài liệu được liên kết và hiển thị liền mạch.
- [2026-07-17] **Cập nhật Báo cáo Kết quả từng Agent vào Tài liệu tri thức**:
  * Thêm file `Báo cáo Nhật ký Công việc.md` làm Proxy Note nhúng trực tiếp báo cáo logs từ `output/4_daily_logs_report.md` (được thêm vào whitelist vệ sinh của pipeline).
  * Tự động hóa chèn liên kết báo cáo động (nhúng `![[...]]` tương đối) vào cuối tài liệu tri thức của cả 5 Agent (Agent 1, 2, 3, 4, và Lead). Giờ đây, khi mở các file Agent này trong Obsidian, kết quả kiểm toán và phân tích thực tế mới nhất sẽ hiển thị trực tiếp bên dưới phần mô tả kiến trúc.
- [2026-07-17] **Chốt Quy trình Quản lý Dữ liệu và Tối ưu Token**:
  * Đã thống nhất áp dụng Phương án 1: Lập trình hóa & Tổng hợp đa tầng. Giữ nguyên định dạng gốc nhị phân/quan hệ cho các dữ liệu lớn (Excel, SQL) để xử lý tính toán qua Python (Pandas/SQL), đảm bảo độ chính xác số học tuyệt đối và tránh LLM quá tải token do đọc trực tiếp văn bản thô.
  * Chỉ Markdown hóa các văn bản ngắn như quy tắc, quy chế và định mức (<150 dòng) để Agent đọc trực tiếp.
  * Thiết lập quy trình tổng kết liên phiên bắt buộc vào `Super Memory.md` và `walkthrough.md` để Agent tải bối cảnh ở đầu phiên làm việc sau chỉ với dưới 1000 tokens.
  * Xuất bản tài liệu thiết kế quy trình chính thức tại `docs/plans/Plan - 2026-07-17 Input and Session Token Optimization.md` và đồng bộ liên kết vào MOC.
- [2026-07-17] **Đồng nhất Giao diện SPA Master Dashboard (Agent 5)**:
  * Khắc phục lỗi lệch font chữ, màu sắc và kiểu dáng bảng/card giữa các tab con khi nhúng vào SPA Master Portal. Thiết lập hệ thống CSS Design System toàn cục ghi đè bo góc `1.25rem` cho các bảng và card của tab con, enforce font-family `Fira Sans` toàn cục, đồng bộ màu sắc Slate/Indigo/Indigo-Dark sang trọng.
  * Sửa lỗi cú pháp ngoặc nhọn f-string trong script `generate_unified_dashboard.py` (escape `{` thành `{{`).
  * Xác minh thư mục `output/` chứa đầy đủ 8 file kết quả của từng Agent và xếp hạng V2 (`1_kpi_report.html`, `2_class_predictions_dashboard.html`, `3_gvtg_violations_report.html`, `4_daily_logs_report.html`, `4_daily_logs_report.md`, `5_master_evaluation_dashboard.html`, `kpi_classification_report.html/md`).
- [2026-07-18] **Sửa lỗi lệch môn học hiện tại & Tối ưu thời gian khởi động MySQL**:
  * Phát hiện MySQL Server 3307 cần tới 11 giây để khởi chạy hoàn tất (đặc biệt sau bước XA crash recovery), trong khi script `auto_update_kpi.ps1` chỉ sleep 5 giây làm hỏng tiến trình tự động hóa. Đã chuyển sang khởi chạy MySQL trực tiếp dưới dạng background task ổn định.
  * Đồng bộ hóa chính xác cấu hình môn học hiện tại trong `analyze_student_risk_real.py` và `run_cross_validation_predictions.py`: Khóa KS25 CNTT chuyển sang FastAPI (ID 217), KS24 CNTT chuyển sang AI Application (ID 214), và KS25 QTKD chuyển sang PRJ302 (ID 218).
  * Chạy lại thành công script `run_cross_validation_predictions.py` để cập nhật file tĩnh `predictions_cv_data.json` và thực thi toàn bộ pipeline đồng bộ hóa báo cáo, dashboard mới nhất đến ngày 17/07.
- [2026-07-18 22:43] **Lọc sheet active & Tích hợp export_and_upload**:
  * Phát hiện Tab 1 hiển thị dữ liệu cũ vì script `export_and_upload.py` (chuyển đổi `report_kpi_gv_tg.md` sang HTML) không nằm trong pipeline và copy sai tên `kpi_report.html`.
  * Đã sửa `run_pipeline.py` tích hợp thêm bước chạy `export_and_upload.py` và copy output `report_kpi_gv_tg.html` đè lên `1_kpi_report.html` để cập nhật Tab 1.
  * Cập nhật `target_sheets` chỉ xử lý các sheet đang active thực tế (`KS25_Python_Web` và `KS25_QTKD_PRJ302`), bỏ qua khối KS24 (đang làm dự án hè) và các môn cũ, giúp báo cáo sạch dữ liệu cũ.
- [2026-07-18 23:15] **Khôi phục Báo cáo Agent 1 & Động hóa Thời gian**:
