...
output_path = 'output/reports/report_agent1.md'
...
cohort_sheets = {
    'KS24_HN': ['KS24-JavaAdvance', 'KS24_JavaWeb', 'KS24_JWS', 'KS24_AI'],
    ...
    'QTKD': ['KS25_QTKD_M103', 'KS25_QTKD_M104', 'KS25_QTKD_DTB201', 'KS25_QTKD_DTB202', 'KS25_QTKD_PRJ302']
}
for cohort, sheets in cohort_sheets.items():
    for sheetname in sheets:
        ...
        for (cname, role), courses in class_course_data.items():
            ...
            if cohort == 'HCM' and not ('HCM' in cname and 'CNTT' in cname): continue
            if cohort == 'QTKD' and not 'QTKD' in cname: continue
            
            cc_vals.append(courses[sheetname]['cc_avg'])
            bt_vals.append(courses[sheetname]['bt_avg'])
            el_vals.append(courses[sheetname]['el_avg'])
            
        course_name = sheetname.replace('KS24-', '').replace('KS24_', '').replace('KS25_', '').split('_')[-1]
        trends_data[cohort]['courses'].append(course_name)
        trends_data[cohort]['cc'].append(round(float(np.mean(cc_vals)) if cc_vals else 0.0, 2))
        trends_data[cohort]['bt'].append(round(float(np.mean(bt_vals)) if bt_vals else 0.0, 2))
        trends_data[cohort]['el'].append(round(float(np.mean(el_vals)) if el_vals else 0.0, 2))

trends_data['compare'] = {}
for gkey, dict_key in [('KS25_CNTT_HN', 'HN'), ('KS25_CNTT_HCM', 'HCM'), ('KS25_QTKD_HN', 'QTKD')]:
    stats = weekly_stats.get(gkey, {'curr': {}, 'prev': {}})
    curr_cc = np.mean([x['metrics']['Chuyên cần'] for x in stats['curr'].values()]) if stats['curr'] else 0.0
    curr_bt = np.mean([x['metrics']['Bài tập'] for x in stats['curr'].values()]) if stats['curr'] else 0.0
    curr_el = np.mean([x['metrics']['Elearning'] for x in stats['curr'].values()]) if stats['curr'] else 0.0
    
    prev_cc = np.mean([x['metrics']['Chuyên cần'] for x in stats['prev'].values()]) if stats['prev'] else 0.0
    prev_bt = np.mean([x['metrics']['Bài tập'] for x in stats['prev'].values()]) if stats['prev'] else 0.0
    prev_el = np.mean([x['metrics']['Elearning'] for x in stats['prev'].values()]) if stats['prev'] else 0.0
...
def generate_cohort_section(cohort_id, weekly_stats_group):
    # Calculate averages
    curr_vals = {m: np.mean([x['metrics'][m] for x in weekly_stats_group['curr'].values()]) if weekly_stats_group['curr'] else 0.0 for m in ['Chuyên cần', 'Bài tập', 'Elearning']}
    prev_vals = {m: np.mean([x['metrics'][m] for x in weekly_stats_group['prev'].values()]) if weekly_stats_group['prev'] else 0.0 for m in ['Chuyên cần', 'Bài tập', 'Elearning']}
...
<div style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 20px; margin: 20px 0; backdrop-filter: blur(10px);">
    <h4 style="margin-top:0; color:#e2e8f0; display:flex; align-items:center; gap:8px;">
        <span style="font-size:1.2rem;">📌</span> AI Action Plan (Đề xuất hệ thống)
    </h4>
...
<div style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 12px; padding: 16px; margin: 20px 0; display: flex; align-items: center; gap: 12px;">
    ...
    <div>
        <strong style="color: #6ee7b7;">AI Insight:</strong> Kỷ luật của khóa học {cohort_id} đang được duy trì rất ổn định so với tuần trước. Không cần can thiệp khẩn cấp.
    </div>
</div>
"""
...
html_cards = "<div class='staff-grid' style='display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px; margin-top: 20px;' markdown='1'>\n"
for item in sorted(evaluated_staff, key=lambda x: x['cmi'], reverse=True):
...
<div style="font-size: 0.8rem; color: #94a3b8; margin-top: 4px;">Vai trò: {item['role']} - {item['classes_count']} Lớp</div>
</div>
