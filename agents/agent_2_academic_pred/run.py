...
def predict_class_pass_rate(cursor, cid, co_id, class_course_seq, excel_data, cname, coname, batch):
    ...
    for s in calibrated_students:
        ...
        elif p_final < 30.0 and (att_val > 15.0 or (100.0 - hw_val) > 20.0):
            risk_level = "RED" # CAO - Học lực quá yếu VÀ Vi phạm kỷ luật
            reasons.append(f"Xác suất đỗ quá thấp ({p_final:.1f}%) kèm vi phạm KL")
        elif (att_val > 15.0 and (100.0 - hw_val) > 30.0) or (p_final < 40.0 and el_val >= 2.0) or (att_val > 20.0) or ((100.0 - hw_val) > 40.0):
...
def main():
    ...
    md_report_path = "output/reports/report_agent2.md"
    ...
    md_care_path = "output/reports/student_care_list_multi_level.md"
    ...
    subprocess.run([sys.executable, "agents/agent_2_academic_pred/generate_report.py"])
