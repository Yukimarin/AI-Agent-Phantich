...
def main():
    ...
    res = subprocess.run([sys.executable, "agents/master/agent_5_master_portal/generate_kpi_report.py"], capture_output=True, text=True, encoding="utf-8")
    print(res.stdout)
    if res.returncode != 0:
    ...
    res = subprocess.run([sys.executable, "agents/master/agent_5_master_portal/generate_unified_dashboard.py"], capture_output=True, text=True, encoding="utf-8")
    print(res.stdout)
    if res.returncode != 0:
