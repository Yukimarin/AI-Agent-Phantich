...
def main():
    ...
    res = subprocess.run([sys.executable, "agents/agent_3_ops_discipline/analyze_gvtg_violations.py"], capture_output=True, text=True, encoding="utf-8")
    print(res.stdout)
    if res.returncode != 0:
    ...
    res = subprocess.run([sys.executable, "agents/agent_3_ops_discipline/generate_report.py"], capture_output=True, text=True, encoding="utf-8")
    print(res.stdout)
    if res.returncode != 0:
