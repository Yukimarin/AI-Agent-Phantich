...
output_html_path = "custom_reports/4_daily_logs_report_director.html"
output_md_path = "custom_reports/4_daily_logs_report_director.md"
  ...
  <script>
    ...
    function calculateSystemState() {
      ...
      projectsData.forEach(p => {
        ...
        p.issues.forEach(iss => {
          ...
          if (isDone) {
            ...
            // Rule 2: Logic Lọc Thời gian
            let shouldCountActive = true;
            if (iss.dueDate) {
              const dueStr = iss.dueDate.slice(0, 10);
              if (dueStr < todayRef) {
                // Overdue tasks are always counted
                stats.overdue += 1;
                iss.is_overdue = true;
                iss.is_urgent = false;
              } else {
                ...
                try {
                  ...
                  // Filter active tasks based on time range if not overdue
                  if (currentTimeRange === "daily") {
                    if (dueStr !== todayRef) shouldCountActive = false;
                  } else if (currentTimeRange === "weekly") {
                    if (diffDays > 7) shouldCountActive = false; // beyond this week
                  } else if (currentTimeRange === "monthly") {
                    if (diffDays > 30) shouldCountActive = false; // beyond this month
                  }
                } catch(e) {
                  iss.is_urgent = false;
                }
            ...
            if (shouldCountActive || iss.is_overdue) {
              stats.active += 1;
            }
            if (shouldCountActive && !iss.is_overdue) {
      ...
      Object.keys(personnelData).forEach(name => {
        ...
        if (currentTimeRange === "daily") {
          ...
          calculatedHours = pMeta.weekly.hours;
          reportedDays = pMeta.weekly.reported_days;
          expectedDays = pMeta.weekly.expected_days;
          workScore = pMeta.weekly.score;
        } else if (currentTimeRange === "monthly") {
          calculatedHours = pMeta.monthly.hours;
          reportedDays = pMeta.monthly.reported_days;
          expectedDays = pMeta.monthly.expected_days;
          workScore = pMeta.monthly.score;
        }
    ...
    function populateDateSelect() {
      ...
      if (currentTimeRange === "daily") {
        ...
        const allDates = [...datesMonthly].sort((a,b) => b.localeCompare(a));
        allDates.forEach(d => {
