...
def patch():
    file_path = "agents/advanced/management_audit/generate_qldt_report.py"
    ...
    start_anchor = "def analyze_and_generate():"
    end_anchor = "    # We define raw HTML structure as a normal string (NOT f-string) to avoid escape problems with { } in CSS/JS"
    ...
    print(f"Found start_idx at {start_idx}, end_idx at {end_idx}")

    # New implementation of analyze_and_generate()
    new_impl = """def analyze_and_generate():
...
"""
    ...
    patched_content = content[:start_idx] + new_impl + content[end_idx:]

    with open(file_path, "w", encoding="utf-8") as f:
        f.write(patched_content)

    print("Successfully patched generate_qldt_report.py backend logic!")
