...
def main():
    ...
    for row in filtered_rows:
        ...
        if isinstance(raw_date, datetime):
            ...
            try:
                # Thử parse string ngày
                if isinstance(raw_date, str):
                    # Thường có dạng "YYYY-MM-DD" hoặc "YYYY-MM-DD HH:MM:SS"
    ...
    for row in unique_rows:
        ...
        if not_cared_students:
            # Gộp lỗi chăm sóc theo ca dạy để tránh quá nhiều lỗi vụn vặt
    ...
    print(f"\n================================================================================")
    print(f"BÁO CÁO VI PHẠM TÁC NGHIỆP GV/TG KHÓA KS25 MÔN PYTHON WEB - BẢN DEMO")
    print(f"================================================================================")
    
    output_json_path = "data/vi_pham_gvtg.json"
    ...
    if not violations:
        print("Chúc mừng! Không phát hiện vi phạm nào.")
        ...
        print(f"{'Date':<12} | {'Class':<15} | {'Session':<10} | {'Instructor':<20} | {'Role':<6} | {'Error':<6}")
        print("-" * 80)
        for v in violations:
            print(f"{v['Date']:<12} | {v['Class']:<15} | {v['Session']:<10} | {v['Instructor']:<20} | {v['Role']:<6} | {v['Error']:<6}")
        
        # Ghi Markdown
        with open(output_report_path, "w", encoding="utf-8") as f:
            f.write("# Báo cáo Vi phạm Kỷ luật tác nghiệp GV/TG khóa KS25 - Môn Python Web (Bản Demo)\n\n")
            f.write(f"*Thời gian đối chiếu: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}*\n\n")
            f.write(f"> [!IMPORTANT]\n")
            f.write(f"> Báo cáo này quét và đối chiếu tự động 6 tiêu chí vi phạm tác nghiệp theo quy định chế tài tháng 06/2026. Tên nhân sự vi phạm được quy đổi theo phân công quản lý lớp chính thức từ Agent 1.\n\n")
